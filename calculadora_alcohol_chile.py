...
with st.sidebar:
    ...
    hora_manejo_input = st.time_input(
        "Hora objetivo para manejar",
        value=ahora.replace(hour=min(ahora.hour + 3, 23), minute=0).time()
    )
...
if not df.empty and gramos_total > 0:
    ...
    # Separar curva en pasado y futuro
    df_pasado = df[df["tiempo"] <= ahora]
    df_futuro = df[df["tiempo"] >= ahora]

    if not df_pasado.empty:
        fig.add_trace(go.Scatter(
            x=df_pasado["tiempo"], y=df_pasado["bac"],
    ...
    if not df_futuro.empty:
        fig.add_trace(go.Scatter(
            x=df_futuro["tiempo"], y=df_futuro["bac"],
    ...
    fig.add_vline(x=ahora.strftime("%Y-%m-%d %H:%M:%S"), line_dash="solid",
    ...
    fig.add_vline(x=dt_manejo.strftime("%Y-%m-%d %H:%M:%S"), line_dash="dash",
                  line_color="rgba(52,152,219,0.6)", line_width=1.5,
                  annotation_text=f"  🚗 {dt_manejo.strftime('%H:%M')}",
    ...
    for ev in eventos_consumo:
        color_marca = "rgba(52,152,219,0.3)" if ev["es_futuro"] else "rgba(192,57,43,0.3)"
        fig.add_vline(x=ev["dt"].strftime("%Y-%m-%d %H:%M:%S"), line_dash="dot",
                      line_color=color_marca, line_width=1)
