...
Modelo de Widmark corregido + hora de consumo por trago.
"""
...
.stButton>button {
    ...
    font-size: 1rem; padding: 0.6rem 2rem;
...
# ─────────────────────────────────────────────
# DATOS: Tragos chilenos de matrimonio
# ─────────────────────────────────────────────
TRAGOS = {
...
LIMITE_CONDUCIR = 0.30   # g/L → infracción grave
LIMITE_EBRIEDAD = 0.50   # g/L → delito

# ─────────────────────────────────────────────
# Funciones
# ─────────────────────────────────────────────

def gramos_etanol(ml: float, grad: float) -> float:
    """
    Gramos de etanol puro.
    g = volumen_ml × (graduación/100) × densidad_etanol(0.789 g/ml)
    Ejemplo: 355ml × 0.05 × 0.789 = 14.0g  ✓
    """
    return ml * (grad / 100.0) * 0.789


def factor_widmark(peso_kg: float, altura_cm: float, edad: int, sexo: str) -> float:
    """
    Factor r de Widmark via TBW de Watson (litros de agua corporal).
    r = TBW / peso_kg  (fracción del cuerpo que distribuye el alcohol)
    Hombres típico ~0.68, Mujeres ~0.55
    """
    if sexo == "Masculino":
...
def bac_de_trago(gramos: float, factor_comida: float,
                 peso_kg: float, r: float,
                 horas_desde_consumo: float) -> float:
    """
    BAC en g/L para UN evento de consumo (puede ser 1 o varios tragos tomados juntos).

    Fórmula de Widmark:
        BAC_pico (g/L) = (A_absorbida [g]) / (r × peso_kg [kg])
        Nota: g/kg = g/L de sangre (densidad sangre ≈ 1.055, factor ya incluido en r)

    Eliminación hepática β = 0.15 g/L/hora (promedio; rango 0.10-0.20)
    Tiempo absorción ≈ 30-90 min según comida.

    BAC_actual = max(0, BAC_pico - β × max(0, t - t_absorcion))
    """
    beta = 0.15  # g/L por hora

    # Absorción más lenta con comida
    t_absorcion = 0.5 + 0.5 * (1.0 - factor_comida)  # 0.5h vacío → 1.0h lleno

    gramos_absorbidos = gramos * factor_comida  # comida reduce biodisponibilidad

    # BAC pico en g/L
    bac_pico = gramos_absorbidos / (r * peso_kg)

    # Tiempo efectivo de eliminación
    t_elim = max(0.0, horas_desde_consumo - t_absorcion)

    return max(0.0, bac_pico - beta * t_elim)


def bac_total(eventos: list, factor_comida: float,
              peso_kg: float, r: float,
              dt_actual: datetime) -> float:
    """
    Suma los BAC de todos los eventos de consumo.
    Los BAC son aditivos cuando se calculan sobre la misma persona.
    eventos = lista de dicts {"gramos": float, "dt": datetime}
    """
    total = 0.0
    for ev in eventos:
        horas = (dt_actual - ev["dt"]).total_seconds() / 3600.0
        if horas < 0:
            horas = 0.0
        total += bac_de_trago(ev["gramos"], factor_comida, peso_kg, r, horas)
    return round(total, 3)


def tiempo_para_limite(bac: float, limite: float) -> float:
    """Horas para bajar de bac al limite (β = 0.15 g/L/h)."""
    if bac <= limite:
        return 0.0
    return (bac - limite) / 0.15


def curva_bac(eventos: list, factor_comida: float,
              peso_kg: float, r: float,
              dt_actual: datetime, ventana_h: int = 14) -> pd.DataFrame:
    """Genera puntos de la curva BAC desde el primer trago hasta ventana_h horas después."""
    if not eventos:
        return pd.DataFrame()
    ...
    for min_offset in range(0, ventana_h * 60 + 1, 10):
        ...
        b = bac_total(eventos, factor_comida, peso_kg, r, dt)
...
with st.sidebar:
    ...
    hora_actual_input = st.time_input("¿Qué hora es ahora?",
                                      value=datetime.now().time())
    hoy = datetime.today().date()
    dt_actual = datetime.combine(hoy, hora_actual_input)

    r = factor_widmark(peso, altura, edad, sexo)
    st.markdown("---")
    st.caption(f"Factor Widmark (r): **{r:.3f}**")
    if sexo == "Masculino":
        st.caption("Tasa eliminación: **0.15 g/L/h** (promedio)")
    else:
        st.caption("Tasa eliminación: **0.15 g/L/h** (promedio)")

# ── Main: ingreso de tragos ────────────────
st.markdown('<div class="section-header">🍹 Tragos Consumidos</div>', unsafe_allow_html=True)
st.markdown("Para cada trago, ingresa la **cantidad** y la **hora aproximada** en que lo tomaste.")

# Tabla de tragos con cantidad + hora
eventos_consumo = []  # lista de {"nombre", "cantidad", "hora", "gramos"}

for trago_nombre, t_info in TRAGOS.items():
    ...
    with st.container():
        ...
        with c1:
            st.markdown(
                f"<div style='padding-top:0.5rem; font-size:0.95rem'>"
                f"<b>{trago_nombre}</b><br>"
                f"<small style='color:#888'>{t_info['ml']}ml · {t_info['grad']}° · {g_unit:.1f}g alcohol c/u</small>"
                f"</div>",
                unsafe_allow_html=True
            )
        with c2:
            cantidad = st.number_input(
                "Cantidad", min_value=0, max_value=20, value=0, step=1,
                key=f"cant_{trago_nombre}", label_visibility="collapsed"
            )
        with c3:
            hora_trago = st.time_input(
                "Hora", value=datetime.now().replace(hour=20, minute=0).time(),
                key=f"hora_{trago_nombre}", label_visibility="collapsed"
            )

        if cantidad > 0:
            dt_trago = datetime.combine(hoy, hora_trago)
            # Si la hora del trago es posterior a "ahora", asumir que fue el día anterior
...
if gramos_total == 0:
    ...
    # Verificación rápida (muestra en sidebar)
    with st.sidebar:
        st.markdown("---")
        st.markdown("### 🧮 Verificación")
        st.caption(f"Gramos totales de etanol: **{gramos_total:.1f} g**")
        bac_pico_ref = gramos_total / (r * peso)
        st.caption(f"BAC pico teórico (sin eliminación): **{bac_pico_ref:.3f} g/L**")

    # BAC actual
    bac_act = bac_total(eventos_consumo, factor_comida, peso, r, dt_actual)
    ...
    with col3:
        if horas_conducir == 0:
            ...
            st.metric("🚗 Para manejar",
                      f"{h}h {m:02d}m",
                      delta=f"OK a las {dt_ok_conducir.strftime('%H:%M')}")
    with col4:
        if horas_ebriedad == 0:
            st.metric("⚖️ Ebriedad legal", "No aplica", delta="No en estado ebriedad")
        else:
            h2, m2 = int(horas_ebriedad), int((horas_ebriedad % 1) * 60)
            st.metric("⚖️ Fuera de ebriedad",
                      f"{h2}h {m2:02d}m",
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    if bac_act == 0.0:
        st.markdown('<div class="status-safe">✅ Sin alcohol en sangre — Puedes conducir.</div>',
                    unsafe_allow_html=True)
    ...
    # ── Desglose ───────────────────────────
    st.markdown('<div class="section-header">🍹 Desglose</div>', unsafe_allow_html=True)
    cols = st.columns(3)
    for i, ev in enumerate(eventos_consumo):
        with cols[i % 3]:
            st.markdown(f"""
            <div style='background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);
                        border-radius:10px;padding:0.8rem 1rem;margin:0.3rem 0;'>
                <b>{ev['nombre']}</b><br>
                <small>× {ev['cantidad']} · {ev['dt'].strftime('%H:%M')} hrs</small><br>
                <small style='color:#C0392B'>{ev['gramos']:.1f} g alcohol total</small>
            </div>
            """, unsafe_allow_html=True)
...
> Chile mide en **g/L**. Tasa de eliminación promedio: **0.15 g/L/hora**.
    ...
    ⚠️ <b>AVISO:</b> Estos son valores estimados basados en el modelo de Widmark.
    El BAC real varía según metabolismo individual, medicamentos, hidratación y otros factores.
