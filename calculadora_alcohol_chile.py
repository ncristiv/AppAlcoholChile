...
def gramos_etanol(ml: float, grad: float) -> float:
    ...
    g = volumen_ml × (graduacion/100) × densidad_etanol(0.789 g/ml)
    Ej: 355ml × 0.05 × 0.789 = 14.0 g  ✓
    """
...
def factor_widmark(peso_kg: float, altura_cm: float, edad: int, sexo: str) -> float:
    """
    Factor r de Widmark via TBW de Watson.
    r = TBW / peso_kg  (fracción del cuerpo que distribuye el alcohol)
    Hombres tipico ~0.68, Mujeres ~0.55
    """
    if sexo == "Masculino":
...
def bac_de_evento(gramos: float, factor_comida: float,
                  peso_kg: float, r: float,
                  horas_desde_consumo: float) -> float:
    """
    BAC en g/L para un evento de consumo.

    Fórmula de Widmark:
        BAC_pico [g/L] = A_absorbida [g] / (r × peso_kg [kg])
        (g/kg ≈ g/L porque densidad sangre ~1.055 ya absorbida en r empírico)

    Eliminacion hepatica β = 0.15 g/L/hora
    Tiempo absorcion = 0.5h (vacio) a 1.0h (lleno)
    """
    beta = 0.15
...
def bac_total_fn(eventos: list, factor_comida: float,
                 peso_kg: float, r: float,
...
with st.sidebar:
    ...
    st.caption("Tasa eliminación: **0.15 g/L/h**")

# ── Main ─────────────────────────────────────
st.markdown('<div class="section-header">🍹 Tragos Consumidos</div>', unsafe_allow_html=True)
st.markdown("Ingresa la **cantidad** y la **hora** en que tomaste cada trago.")

# Encabezado de columnas
h1, h2, h3 = st.columns([3, 1, 2])
h1.markdown("<small style='color:#888'>Trago · contenido · alcohol por unidad</small>", unsafe_allow_html=True)
h2.markdown("<small style='color:#888; text-align:center; display:block'>Cantidad</small>", unsafe_allow_html=True)
h3.markdown("<small style='color:#888'>Hora de consumo</small>", unsafe_allow_html=True)
...
for trago_nombre, t_info in TRAGOS.items():
    ...
    with c1:
        st.markdown(
            f"<div style='padding-top:0.45rem;font-size:0.93rem'>"
...
st.button("🔍 CALCULAR MI NIVEL DE ALCOHOL", use_container_width=True)
...
if gramos_total == 0:
    ...
    # Panel verificación en sidebar
    with st.sidebar:
        st.markdown("---")
        st.markdown("### 🧮 Verificación")
        bac_pico_ref = (gramos_total * factor_comida) / (r * peso)
    ...
    for i, ev in enumerate(eventos_consumo):
        with cols[i % 3]:
            ...
            <div style='background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.07);
                ...
                <small style='color:#C0392B'>{ev['gramos']:.1f} g etanol total</small>
            </div>
            """, unsafe_allow_html=True)
