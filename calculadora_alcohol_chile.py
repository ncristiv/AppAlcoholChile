...
LIMITE_SEGURO   = 0.20   # nuevo límite conservador
...
def hora_ok(eventos, fc, peso_kg, r, desde, limite):
    """
    Encuentra la primera hora en que el BAC baja DEFINITIVAMENTE bajo el límite,
    es decir, el último instante en que el BAC supera el límite, más un minuto.

    Esto cubre el caso de tragos futuros: si el BAC baja a 0 ahora pero
    luego sube por un trago planificado, no se reporta 'ya puedes' prematuramente.

    Estrategia:
    1. Escanear las próximas 36h en pasos de 5 minutos.
    2. Registrar el ÚLTIMO momento en que BAC >= límite.
    3. Retornar ese momento + 1 minuto (primer instante bajo el límite).
    4. Si nunca supera el límite → retornar None (ya puede manejar).
    """
    ultimo_sobre_limite = None
    dt = desde
    for _ in range(36 * 12):   # 36h en pasos de 5 min
        b = bac_en_momento(eventos, fc, peso_kg, r, dt)
        if b >= limite:
            ultimo_sobre_limite = dt
        dt += timedelta(minutes=5)

    if ultimo_sobre_limite is None:
        return None  # nunca superó el límite → ya puede manejar

    # Afinar al minuto exacto desde el último punto sobre el límite
    dt = ultimo_sobre_limite
    for _ in range(10):        # refinar hasta 10 min después
...
for fila in st.session_state.filas:
    ...
    with c4:
        # Default siempre Pasado (False) salvo que se haya guardado explícitamente Futuro
        _default_futuro = bool(st.session_state.get(f"futuro_{fid}", False))
        tipo_sel = st.radio(
            "Tipo", ["✅ Pasado", "🔮 Futuro"],
            index=1 if _default_futuro else 0,
            key=f"radio_{fid}",
            horizontal=True,
...
if gramos_total > 0 and gramos_pasado == 0:
    st.warning("⚠️ **Todos los tragos están marcados como Futuro** — el BAC actual es 0. Si ya tomaste alguno, cámbialo a **✅ Pasado**.")
...
with m3:
    if dt_ok_legal:
        diff_l = dt_ok_legal - ahora
        h_l = diff_l.seconds//3600; m_l = (diff_l.seconds%3600)//60
        st.metric("⚖️ Manejar legal (0.30 g/L)", dt_ok_legal.strftime("%H:%M"),
                  delta=f"en {h_l}h {m_l:02d}m")
    else:
        lbl_l = "¡Ya puedes!" if bac_ahora > 0 else ("Solo futuros" if gramos_futuro > 0 else "Sin alcohol")
        st.metric("⚖️ Manejar legal (0.30 g/L)", lbl_l, delta="✅")
with m4:
    if dt_ok_seguro:
        diff_s = dt_ok_seguro - ahora
        h_s = diff_s.seconds//3600; m_s = (diff_s.seconds%3600)//60
        st.metric("🟢 Manejar seguro (0.20 g/L)", dt_ok_seguro.strftime("%H:%M"),
                  delta=f"en {h_s}h {m_s:02d}m")
    else:
        lbl_s = "¡Ya puedes!" if bac_ahora > 0 else ("Solo futuros" if gramos_futuro > 0 else "Sin alcohol")
        st.metric("🟢 Manejar seguro (0.20 g/L)", lbl_s, delta="✅")

st.markdown("<br>", unsafe_allow_html=True)

# Estado actual
if bac_ahora < LIMITE_SEGURO:
    st.markdown(f'<div class="status-safe">✅ BAC {bac_ahora:.3f} g/L — Bajo el límite seguro (0.20 g/L). Apto para conducir con total confianza.</div>', unsafe_allow_html=True)
elif bac_ahora < LIMITE_CONDUCIR:
    ok_s = dt_ok_seguro.strftime("%H:%M") if dt_ok_seguro else "—"
    st.markdown(f'<div class="status-ok">🟡 BAC {bac_ahora:.3f} g/L — Bajo el límite legal, pero sobre 0.20 g/L. Puedes conducir sin infracción. Bajo límite seguro a las {ok_s}.</div>', unsafe_allow_html=True)
...
if bac_al_manejar < LIMITE_SEGURO:
    ...
    st.markdown(f'<div class="status-ok">🚗 A las {dt_manejo.strftime("%H:%M")} tu BAC será ~{bac_al_manejar:.3f} g/L — 🟡 Sin infracción legal, pero aún sobre 0.20 g/L.</div>', unsafe_allow_html=True)
else:
    if dt_ok_legal:
        ok_txt = f"Podrás manejar a las {dt_ok_legal.strftime('%H:%M')}."
    elif bac_ahora == 0 and bac_al_manejar > LIMITE_CONDUCIR:
        ok_txt = "⚠️ BAC actual es 0: revisa que los tragos ya tomados estén marcados como Pasado."
    else:
        ok_txt = "No podrás manejar en las próximas 24h."
...
if not df.empty and gramos_total > 0:
    ...
    for y_val, col_h, lbl in [
        (LIMITE_SEGURO,   "#1abc9c", f"0.20 g/L — Límite seguro"),
...
| BAC (g/L) | Situación |
|---|---|
| < 0.20 | 🟢 **Límite seguro** — recomendado por esta app |
| 0.20 – < 0.30 | 🟡 Legal sin infracción, pero con alcohol detectable |
